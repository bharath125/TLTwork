...
export class OnboardingComponent implements OnDestroy, AfterViewInit {
  ...
  private getLightData(): OnboardingLightData | null {
    ...
    const scrollTop =
      win.scrollY || doc.documentElement.scrollTop || doc.body.scrollTop;
    const scrollLeft =
      win.scrollX || doc.documentElement.scrollLeft || doc.body.scrollLeft;
  ...
  private updatePosition(): void {
    ...
    const lightStyle = (
      this.el.nativeElement.querySelector('.onboarding__light') as HTMLElement
    ).style;
  ...
  private updatePrevElStatus(status: boolean): void {
    if (this.prevSelectorEl) {
      this.prevSelectorEl.classList[status ? 'add' : 'remove'](
